resource "null_resource" "update_admin_password" {
  provisioner "local-exec" {
    command = <<EOT
      pod=$(kubectl get pods -n ${var.namespace} --selector app.kubernetes.io/name=argocd-server --output jsonpath='{.items[0].metadata.name}')
      password=$(kubectl exec -n ${var.namespace} $pod -c server -- argocd admin initial-password | grep -m1 -v '^$')
