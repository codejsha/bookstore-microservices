...
func TestPointRepository_poolExhausted_applyPointChangeStopsAtRequestDeadline(t *testing.T) {
	db := newExhaustedPoolDB(t)
	r := &pointRepository{q: dao.Use(db), db: db}

	assertStopsAtRequestDeadline(t, func(ctx context.Context) error {
		_, err := r.ApplyPointChange(ctx, "user-1", 10, "EARN", nil)
		return err
	})
}
