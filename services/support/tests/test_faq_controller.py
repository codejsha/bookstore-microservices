...
def test_create_faq_returns_201(client: TestClient, service: MagicMock) -> None:
    faq = make_faq()
...
def test_get_faq_returns_404(client: TestClient, service: MagicMock) -> None:
    service.get_faq.return_value = None
    response = client.get(f"/api/v1/faqs/{uuid4()}")
    assert response.status_code == 404


def test_get_faq_returns_200(client: TestClient, service: MagicMock) -> None:
    faq = make_faq()
...
def test_get_faq_passes_increment_view_flag(client: TestClient, service: MagicMock) -> None:
...
def test_search_staff_can_request_drafts(client: TestClient, service: MagicMock) -> None:
...
def test_search_non_staff_forced_to_published_only(customer_client: TestClient, service: MagicMock) -> None:
    service.search_faqs.return_value = ([make_faq()], 1)
...
def test_get_faq_non_staff_unpublished_returns_404(customer_client: TestClient, service: MagicMock) -> None:
    service.get_faq.return_value = make_faq(published=False)
    response = customer_client.get(f"/api/v1/faqs/{uuid4()}")
    assert response.status_code == 404


def test_get_faq_staff_unpublished_returns_200(client: TestClient, service: MagicMock) -> None:
    service.get_faq.return_value = make_faq(published=False)
    response = client.get(f"/api/v1/faqs/{uuid4()}")
    assert response.status_code == 200


def test_update_faq_returns_404(client: TestClient, service: MagicMock) -> None:
    service.update_faq.return_value = None
    response = client.patch(f"/api/v1/faqs/{uuid4()}", json={"question": "x"})
    assert response.status_code == 404


def test_update_faq_returns_200(client: TestClient, service: MagicMock) -> None:
    faq = make_faq()
...
def test_delete_faq_returns_204(client: TestClient, service: MagicMock) -> None:
    service.delete_faq.return_value = True
    response = client.delete(f"/api/v1/faqs/{uuid4()}")
    assert response.status_code == 204


def test_delete_faq_returns_404(client: TestClient, service: MagicMock) -> None:
