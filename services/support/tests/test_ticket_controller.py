...
def test_create_ticket_returns_201(client: TestClient, service: MagicMock) -> None:
    ticket = make_ticket()
...
def test_get_ticket_returns_404(client: TestClient, service: MagicMock) -> None:
    service.get_ticket.return_value = None
    response = client.get(f"/api/v1/tickets/{uuid4()}")
    assert response.status_code == 404


def test_get_ticket_returns_200(client: TestClient, service: MagicMock) -> None:
    ticket = make_ticket()
...
def test_update_ticket_returns_404(client: TestClient, service: MagicMock) -> None:
    service.update_ticket.return_value = None
    response = client.patch(f"/api/v1/tickets/{uuid4()}", json={"subject": "x"})
    assert response.status_code == 404


def test_update_ticket_returns_200(client: TestClient, service: MagicMock) -> None:
    ticket = make_ticket()
