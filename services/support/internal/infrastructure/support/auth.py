...
def get_principal(request: Request) -> Principal | None:
    ...
    raw = _decode_payload(payload) if payload else {}
    return Principal(
        sub=sub,
...
def owns(principal: Principal, uid: UUID) -> bool:
    parsed = principal_uid(principal)
    return parsed is not None and parsed == uid
