...
def test_create_freight_returns_201(client: TestClient, service: MagicMock) -> None:
...
def test_get_freight_returns_404_when_missing(client: TestClient, service: MagicMock) -> None:
    service.get_freight.return_value = None
    response = client.get(f"/api/v1/freights/{uuid4()}")
    assert response.status_code == 404


def test_list_freights_passes_filters(client: TestClient, service: MagicMock) -> None:
...
def test_get_by_shipment_returns_404(client: TestClient, service: MagicMock) -> None:
    service.find_by_shipment.return_value = None
    response = client.get(f"/api/v1/freights/shipment/{uuid4()}")
    assert response.status_code == 404


def test_get_by_shipment_returns_200(client: TestClient, service: MagicMock) -> None:
    freight = make_freight()
...
def test_update_status_returns_200(client: TestClient, service: MagicMock) -> None:
...
def test_update_status_returns_404(client: TestClient, service: MagicMock) -> None:
