...
class TestCreateShipment:
    async def test_persists_shipment_with_planned_status(
    ...
    async def test_creates_initial_tracking_event_atomically(
...
class TestGetShipment:
    async def test_returns_shipment_when_found(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
        shipment = make_shipment()
        shipment_repo.find_by_uid.return_value = shipment
        assert await service.get_shipment(shipment.uid) is shipment

    async def test_returns_none_when_not_found(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
        shipment_repo.find_by_uid.return_value = None
        assert await service.get_shipment(uuid4()) is None


class TestListShipments:
    async def test_delegates_to_repository(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
...
class TestAssignCarrier:
    async def test_returns_none_when_shipment_missing(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
        shipment_repo.find_by_uid.return_value = None
        result = await service.assign_carrier(uuid4(), AssignCarrierCommand(carrier_uid=uuid4()))
        assert result is None
        shipment_repo.save.assert_not_called()

    async def test_updates_carrier_and_tracking_number(
        self, service: ShipmentService, shipment_repo: MagicMock
    ) -> None:
        shipment = make_shipment()
        shipment_repo.find_by_uid.return_value = shipment
...
class TestDispatchShipment:
    async def test_transitions_planned_to_dispatched(
        self, service: ShipmentService, shipment_repo: MagicMock, tracking_repo: MagicMock
    ) -> None:
    ...
    async def test_returns_none_when_missing(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
        shipment_repo.find_by_uid.return_value = None
    ...
    async def test_raises_when_status_invalid(
...
class TestPickUpShipment:
    async def test_transitions_dispatched_to_picked_up(
        self, service: ShipmentService, shipment_repo: MagicMock
    ) -> None:
    ...
    async def test_raises_when_status_invalid(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
...
class TestDeliverShipment:
    ...
    async def test_transitions_to_delivered(
        self, service: ShipmentService, shipment_repo: MagicMock, status: ShipmentStatus
    ) -> None:
...
class TestCancelShipment:
    ...
    async def test_transitions_to_cancelled(
        self, service: ShipmentService, shipment_repo: MagicMock, status: ShipmentStatus
    ) -> None:
        shipment_repo.find_by_uid.return_value = make_shipment(status=status)
...
class TestAddTracking:
    async def test_creates_tracking_when_shipment_exists(
        self, service: ShipmentService, shipment_repo: MagicMock, tracking_repo: MagicMock
    ) -> None:
        shipment = make_shipment()
        shipment_repo.find_by_uid.return_value = shipment
    ...
    async def test_raises_when_shipment_missing(self, service: ShipmentService, shipment_repo: MagicMock) -> None:
        shipment_repo.find_by_uid.return_value = None
        command = AddTrackingCommand(
            shipment_uid=uuid4(),
            status=ShipmentStatus.IN_TRANSIT,
...
class TestGetTrackingHistory:
    async def test_returns_repository_results(self, service: ShipmentService, tracking_repo: MagicMock) -> None:
        shipment_uid = uuid4()
