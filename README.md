...
도메인 주도 설계를 통해 비즈니스 도메인을 파악하고 요구사항과 문제를 효과적으로 해결합니다. 핵심 도메인과 다른 도메인들을 식별하여 의사결정을 내리는 데 도움을 줄 수 있습니다.
...
외부 시스템과의 연동에 대한 포트를 인터페이스로 정의하고, 이에 대한 어댑터를 구현하고 연동합니다. 그리고 어플리케이션의 비즈니스 로직의 인터페이스인 유스케이스를 정의하고 서비스(로직)로 구현합니다.<br>
어댑터에서는 유스케이스 또는 핸들러를 호출하여 도메인 모델과 연결됩니다. 각 어플리케이션은 다음과 같이 4단계 또는 3단계 계층 구조로 구성됩니다. 서비스는 핸들러 로직들의 조합을 간략히 하는 파사드 패턴입니다:

- 4단계 계층: 프레젠테이션(API) -- 서비스(비즈니스 로직) -- 핸들러(도메인 모델) -- 데이터 액세스(리포지토리)
- 3단계 계층: 프레젠테이션(API) -- 핸들러(도메인 모델) -- 데이터 액세스(리포지토리)
...
분산 트랜잭션 관리를 위해 적용되며, 중앙 코디네이터인 Conductor OSS에서 워크플로우를 관리합니다. 워크플로우에 정의되어 있는 흐름에 따라 여러 서비스에 걸친 각 단계를 실행하고 결과를 수신하여 다음 단계로 진행합니다. 코드에서 비즈니스 로직이 호출되면 사가 워크플로우가 시작되며, 단계는 워크플로우에 대한 워커에서 태스크로 실행됩니다. 실행한 태스크가 실패하면 보상 로직이 트리거됩니다.<br>
...
Tekton으로 CI/CD 파이프라인을 선언하고 실행합니다. Tekton은 쿠버네티스 네이티브 기반에서 동작하며 태스크와 파이프라인 정의를 재사용할 수 있고 플랫폼 프로바이더에 종속되지 않습니다. 카탈로그는 파이프라인과 태스크를 정의한 리포지토리로, 리소스들은 파이프라인 실행에서 재사용됩니다. 태스크는 파이프라인에서 실행되는 단위이며 파드로 독립적으로 실행됩니다.<br>
...
Kubernetes 클러스터에 인프라 서비스 및 어플리케이션을 배포합니다. 클러스터는 로컬 테스트 편의를 위해 minikube로 구축합니다.

### Helm, Kustomize

인프라 서비스(서버)들은 대부분 Helm 차트로 배포하고 있으며 일부는 Kustomize로 배포됩니다.
...
- Keycloak의 OpenID Connect를 활용하며, Istio를 통해 요청에 대한 인증을 수행합니다.
- cert-manager로 HTTPS 통신을 위한 인증서를 발급하고 관리합니다.
- HashiCorp Vault로 어플리케이션과 인프라의 시크릿 정보를 관리하며 파드에 secret injection되어 사용됩니다.

## 참고

- Building Microservices - O'Reilly (마이크로서비스 아키텍처 구축)
